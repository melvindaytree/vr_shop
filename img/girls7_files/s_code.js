...
s.linkInternalFilters="javascript:,dsw.com,"+document.location.hostname;
s.linkLeaveQueryString=false;
s.linkTrackVars="None";
s.linkTrackEvents="None";
s.usePlugins=true;
function s_doPlugins(p){p.campaign=p.getQueryParam("cm_mmc");
p.eVar2=p.getQueryParam("cm_re");
p.eVar25=p.getQueryParam("cm_lm");
p.eVar31=p.getQueryParam("iq_id");
p.eVar34=p.getQueryParam("adtype");
p.eVar35=p.getQueryParam("mr:adType");
p.eVar36=p.getQueryParam("mr:ad");
p.eVar37=p.getQueryParam("mr:keyword");
if(p.getQueryParam("Ntt")){p.prop11=p.getQueryParam("Ntt")
}var g=p.campaign.indexOf("-_-");
if(g<=-1){g=p.campaign.indexOf("_")
}var n=p.campaign.substring(0,g).toLowerCase();
if(n=="paid"){chancase=1
}else{if(n=="affil"){chancase=2
}else{if(n=="cse"){chancase=3
}else{if(n=="ds"){chancase=4
}else{if(n=="emmkt"){chancase=5
}else{if(n=="emop"){chancase=5
}else{if(n=="emmop"){chancase=5
}else{chancase=6
}}}}}}}switch(chancase){case 1:paidchanpos=g+3;
chantemp=p.campaign.substring(paidchanpos);
p.eVar20="paid search";
p.eVar21=n;
paidchanpos=chantemp.indexOf("-_-");
p.eVar28=chantemp.substring(0,paidchanpos).toLowerCase();
paidchanpos=chantemp.indexOf("-_-");
paidchanpos=paidchanpos+3;
chantemp=chantemp.substring(paidchanpos);
paidchanpos=chantemp.indexOf("-_-");
p.eVar29=chantemp.substring(0,paidchanpos);
paidchanpos=paidchanpos+3;
p.eVar30=chantemp.substring(paidchanpos);
if(p.eVar28=="gdn"){p.eVar20="display"
}break;
case 2:affilchanpos=g+3;
chantemp=p.campaign.substring(affilchanpos);
p.eVar20="affiliate marketing";
p.eVar21=n;
affilchanpos=chantemp.indexOf("-_-");
p.eVar26=chantemp.substring(0,affilchanpos).toLowerCase();
break;
case 3:csechanpos=g+1;
chantemp=p.campaign.substring(csechanpos);
p.eVar20="comparison shopping engine";
p.eVar21=n;
csechanpos=chantemp.indexOf("_");
p.eVar38=chantemp.substring(0,csechanpos).toLowerCase();
csechanpos=csechanpos+1;
chantemp=chantemp.substring(csechanpos);
csechanpos=chantemp.indexOf("_");
p.eVar39=chantemp.substring(0,csechanpos).toLowerCase();
csechanpos=chantemp.indexOf("_")+1;
p.eVar40=chantemp.substring(csechanpos);
break;
case 4:dschanpos=g+1;
chantemp=p.campaign.substring(dschanpos);
p.eVar20="display";
p.eVar21=n;
p.eVar27=chantemp;
break;
case 5:emailchanpos=g+3;
chantemp=p.campaign.substring(emailchanpos);
p.eVar20="email";
p.eVar21=n;
emailchanpos=chantemp.indexOf("-_-");
p.eVar22=chantemp.substring(0,emailchanpos);
emailchanpos=chantemp.indexOf("-_-");
emailchanpos=emailchanpos+3;
chantemp=chantemp.substring(emailchanpos);
emailchanpos=chantemp.indexOf("-_-");
p.eVar23=chantemp.substring(0,emailchanpos).toLowerCase();
emailchanpos=emailchanpos+3;
p.eVar24=chantemp.substring(emailchanpos);
break;
default:break
}p.cpersist=p.getAndPersistValue(p.campaign,"s_cp_persist",7);
if(p.cpersist){p.prop15=p.cpersist+":"+p.pageName
}var m=new Date();
var j=(m.getFullYear());
p.prop5=p.getTimeParting("h","-6",j);
p.eVar7=p.getValOnce(p.prop5,"s_evar7",0);
p.prop6=p.getTimeParting("d","-6",j);
p.eVar8=p.getValOnce(p.prop6,"s_evar8",0);
p.prop7=p.getTimeParting("w","-6",j);
p.eVar9=p.getValOnce(p.prop7,"s_evar9",0);
p.prop8=p.getNewRepeat();
p.eVar10=p.getValOnce(p.prop8,"s_evar10",0);
p.prop9=p.getVisitNum();
p.eVar11=p.getValOnce(p.prop9,"s_evar11",0);
p.prop10=p.getDaysSinceLastVisit("s_lv");
p.eVar12=p.getValOnce(p.prop10,"s_evar12",0);
try{p.prop14=p.getMemberType()
}catch(l){p.prop14="ANONYMOUS"
}if(p.prop4){p.eVar57=p.prop4
}if(p.prop11){p.eVar1=p.prop11
}if(p.prop13){p.eVar15=p.prop13
}if(p.prop12){p.eVar16=p.prop12
}if(p.prop14){p.eVar17=p.prop14
}p.cpersist=p.getAndPersistValue(p.campaign,"s_cp_persist",7);
if(p.cpersist){p.prop15=p.cpersist+":"+p.pageName
}if(p.prop4){var d=p.getPreviousValue(p.prop4,"gpv_pt","")
}if(p.pageName){var k=p.getPreviousValue(p.pageName,"gpv_pn","")
}if(p.events){var o=p.events;
o.split(",").indexOf("event1");
if(d){p.eVar41=d
}if(k){p.eVar42=k
}}if(d){p.eVar44=d
}if(k){p.eVar45=k
}if(p.prop4&&p.prop1&&p.prop4==="Landing Page"&&!p.campaign&&!p.eVar2){p.getAndPersistValue(p.prop1,"s_dp_persist",1)
}if(p.campaign){p.getAndPersistValue("no value","s_dp_persist",1)
}if(p.eVar2&&p.prop4&&(p.prop4==="Collection"||p.prop4==="Product Page")){var i=p.eVar2.split("-_-")[1];
if(!i||!i.toUpperCase().contains("LP")){p.getAndPersistValue("no value","s_dp_persist",1)
}}if(p.events&&p.events.contains("prodView")){if((p.eVar3&&p.eVar3==="BROWSE")||(p.prop4&&p.prop4==="Quick View"&&window.location.pathname.contains("/collection/"))){var b=p.getAndPersistValue(undefined,"s_dp_persist",1);
if(p.prop1&&p.prop1===b){p.products+="|eVar46="+p.prop1
}}}var h=document.location.pathname;
var c=document.location.search;
var f=document.referrer;
var a=/\?.*Ntt=/;
if(p.eVar1){p.eVar64="D=v1";
p.eVar65="Internal Search"
}else{if(h.indexOf("/browse")<0&&f.indexOf("/browse/")<0&&!a.test(c)&&!a.test(f)){p.eVar65=p.eVar64="Non-Search"
}}}s.doPlugins=s_doPlugins;
s.getMemberType=function(){var c=JSON.decode(Cookie.read("DSWstorage"));
var a={A01:"ANONYMOUS",N01:"NONMEMBER",B01:"BASIC",P01:"PREMIER"};
var b=a.A01;
if(typeof(c)=="object"&&c.ldw&&a[c.ldw]){b=a[c.ldw]
}return b
};
...
}}if(q<5||t.indexOf("Opera")>=0||y.indexOf("Opera")>=0){o=s_ft(o)
}if(!z){z=new Object;
if(!r.s_c_in){r.s_c_il=new Array;
r.s_c_in=0
}z._il=r.s_c_il;
z._in=r.s_c_in;
z._il[z._in]=z;
...
(new Function("s","un","pg","ss",o))(z,h,j,A);
return z
};